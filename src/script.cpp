...
cell Script::CE_Convert(std::string input, cell* output, cell size)
{
    int
		len = input.length();
	...
	if (len) {
		...
		for (int i = 0, j = len; i < j; ++i) {
			...
			if (text[i] == '{' && next_space < j && text[next_space] == '}') {
				char EmbledColor[7];
				subc(text.c_str(), i + 1, 6, EmbledColor);
				for (int x = last_check + 1; x < i; x++) {
					if (memchr(vowel, text[x], sizeof(vowel))) {
						...
						if (memchr(spaces, text[x + 1], sizeof(spaces)) || (text[x + 1] == '{' && memchr(spaces, text[x + 9], sizeof(spaces)))) {
							...
							if ((text[x] & 0xE9) == 0xE9 && (text[x - 1] & 0xD5) == 0xD5 && (text[x - 2]) == -95) {
								continue;
							}
						}
						vowel_count++;
				...
				if (vowel_count > 0) shiftLeft(text.data(), EmbledColor, i, vowel_count);
				else if (vowel_count < 0) shiftRight(text.data(), EmbledColor, next_space, abs(vowel_count));
				last_check = next_space;
				i = next_space;
			}
		}
		SetString(output, text, size);
		return 1;
	}
	return 0;
}

cell Script::CE_Convert_Dialog(std::string input, cell* output, int style, cell size)
{
    int
		len = input.length();
	...
	if (len) {
		...
		for (int i = 0, j = len; i < j; ++i) {
			...
			if (text[i] == '{' && next_space < j && text[next_space] == '}') {
				...
				if (vowel_count > 0) shiftLeft(text.data(), EmbledColor, i, vowel_count);
				else if (vowel_count < 0) shiftRight(text.data(), EmbledColor, next_space, vowel_count * -1);
				last_check = next_space;
				i = next_space;
			}
		}
...
cell Script::CE_CountTag(std::string input)
{
    int
		len = input.length(),
		tag_found = 0
...
cell Script::CE_CountVowel(std::string input)
{
    int
		len = input.length(),
		tag_found = 0
